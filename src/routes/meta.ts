...
<div class="wrap">
  ...
  <header>
    ...
    <p style="font-size:13.5px;color:#8a93a6;margin:0 0 20px">⚡ 5 tools · <a style="color:#ff9d47" href="https://registry.modelcontextprotocol.io" target="_blank" rel="noopener">listed in the MCP Registry</a> · works with Claude, Cursor, Windsurf, Cline, Zed, and any MCP client</p>
...
// Text/status pages route through the shared premium shell (routes/shell.ts).
function docPage(title: string, inner: string): string {
	return shell(title, inner, { narrow: true, active: title.toLowerCase() });
}
...
function statusHtml(s: UnderdogStatus): string {
	...
	const fresh =
		s.ageSeconds == null
			...
			: s.ageSeconds < 90
				? `${s.ageSeconds}s ago`
				: `${Math.round(s.ageSeconds / 60)} min ago`;
	...
	const err = s.error
		? `<p class="bad small" style="margin:8px 0 0">Upstream error: ${String(s.error).replace(/[<>&]/g, '')}</p>`
		: '';
	const inner =
...
function tierCard(id: keyof typeof TIERS): string {
	const t = TIERS[id];
	const price =
		t.priceMonthly === null ? 'Custom' : t.priceMonthly === 0 ? 'Free' : `$${t.priceMonthly}<span>/mo</span>`;
